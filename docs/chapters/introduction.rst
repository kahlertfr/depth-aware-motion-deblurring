...
taken picture are affected by this blur. In an image of a planar scene this
blur is the same for each pixel. But scenes with different objects as shown in
figure :ref:`tsu-image` yield different depths. The figure :ref:`tsu-gt` shows
the different depth layers being regions of nearly constant depth. In this
case a near point is blurred more than a distant one. So each depth layer has
...
algorithms to remove spatially-variant blur. However the latter is interesting
...
for motion deblurring with stereo images. It uses spatially-varying Point
Spread Functions (PSF) to deblur the image on each depth level. They overcome
...
to deblurring this algorithm is presented in detail. Where we will focus on
the challenges that had to be solved to be able to deblur an image of a scene
